...
**Multi-tenant** - One account, many inboxes. Assign one inbox per agent and route inbound events by `inbox_id`. To isolate tenants with pods and pod-scoped API keys, see the [Multi-tenancy guide](/guides/multi-tenancy).
...
<Note>
  All API requests must be made over **HTTPS**. Your account-wide API key carries full account access - keep it in environment variables, not in code. To limit a key to a single pod, see [pod-scoped keys](/api-reference/authentication#account-wide-vs-pod-scoped-keys).
</Note>
