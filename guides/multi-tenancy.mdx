...
For white-label setups, scope a [custom domain](/concepts/custom-domains) to the pod so the tenant's inboxes live on their own domain (e.g. `support@meridian.io`). Pass the pod when you add the domain. A pod-scoped domain can only be used by inboxes in that same pod.
...
Publish the DNS records OpenMail returns. The domain verifies on its own — wait until it shows as **Verified**, then create inboxes on it. You can also add and reassign domains in the dashboard under **Settings → Domains** or the pod's **Domains** tab.
