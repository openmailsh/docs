...
For white-label setups, scope a [custom domain](/concepts/custom-domains) to the pod so the tenant's inboxes live on their own domain (e.g. `support@meridian.io`). Pass `podId` (the pod ID or your `clientId`) when you add the domain. A pod-scoped domain can only be used by inboxes in that same pod.

```bash
curl -X POST https://api.openmail.sh/v1/domains \
  -H "Authorization: Bearer $OPENMAIL_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"domain": "meridian.io", "podId": "meridian"}'
# 201 -> { "id", "domain", "podId", "status": "pending", "records": [...] }
```

Publish the returned DNS records. The domain verifies automatically within a couple of minutes — poll `GET /v1/domains/{id}` until `status` is `verified` before creating inboxes on it. You can also add and reassign domains in the dashboard under **Settings → Domains** or the pod's **Domains** tab.
...
| Add a tenant domain (optional) | `POST /v1/domains` (with `podId`) | Account-wide or pod-scoped |
...
| Set the tenant's [sender rules](/concepts/sender-rules) | `PUT /v1/policy?podId={id}` | Account-wide |
| Review rule changes and refused sends | `GET /v1/policy/audit` | Account-wide |
