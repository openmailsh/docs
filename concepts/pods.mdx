...
```
Account
├── Pod  clientId: "acme"          (Acme Corp)
│   ├── Domain: acme.com
│   ├── support@acme.com
│   └── billing@acme.com
└── Pod  clientId: "techstart"     (TechStart Inc)
    ├── Domain: techstart.io
    ├── hello@techstart.io
    └── team@techstart.io
```
...
```
Account
├── Pod  clientId: "client-retail"   (Retail Co)
│   └── info@openmail.sh
└── Pod  clientId: "client-fintech"  (FinTech)
    └── support@openmail.sh
```
...
└── Pod  "sales-agent"
    ├── sales@openmail.sh
    └── outreach@openmail.sh
```

<Note>
  Addresses use `@openmail.sh` here for clarity. Inboxes can also use a verified [custom domain](/concepts/custom-domains).
</Note>
...
curl https://api.openmail.sh/v1/pods/tenant_8f2a \
...
curl "https://api.openmail.sh/v1/inboxes?podId=tenant_8f2a" \
  -H "Authorization: Bearer $OPENMAIL_API_KEY"
...
curl -X DELETE https://api.openmail.sh/v1/pods/tenant_8f2a \
  -H "Authorization: Bearer $OPENMAIL_API_KEY"
```
...
<AccordionGroup>
  ...
  <Accordion title="Can each pod have its own custom domain?">
    Yes. Scope a domain to one pod and create inboxes on it only inside that pod — for example `support@acme.com` under the Acme pod and `hello@techstart.io` under the TechStart pod. You can also add account-wide domains that every pod shares.
  </Accordion>
</AccordionGroup>
