...
- It **cannot read or change [sender rules](/concepts/sender-rules)** at any scope — every `/v1/policy` call is a `403`, so the pod's correspondent boundary cannot be widened or inspected from inside it.
...
<AccordionGroup>
  ...
  <Accordion title="Can I restrict an API key to a single pod?">
    Yes. Mint a **pod-scoped key** with `POST /v1/pods/{id}/api-keys` or from the pod's page in the console. It can only read and send from that pod's inboxes; everything else is out of reach. The token is shown once at creation, and you can revoke the key at any time. See [Pod-scoped API keys](#pod-scoped-api-keys).
  </Accordion>
