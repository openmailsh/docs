...
A pod is an isolated workspace inside your account. Each pod holds its own inboxes, so you can give every one of your end users (or tenants, agents, or projects) a clean, separate slice of OpenMail while keeping a single account and API key.
...
```
Account
├── Pod  clientId: "org_k9v2"       (Lumen Labs)
│   ├── Domain: mail.lumenlabs.co
│   ├── alerts@mail.lumenlabs.co
│   └── noreply@mail.lumenlabs.co
└── Pod  clientId: "org_m3p1"       (Waypoint Health)
    ├── Domain: inbox.waypoint.health
    ├── intake@inbox.waypoint.health
    └── records@inbox.waypoint.health
```
...
```
Account
├── Pod  clientId: "client_bergmann"   (Bergmann & Co)
│   ├── hello@openmail.sh
│   └── projects@openmail.sh
└── Pod  clientId: "client_solstice"   (Solstice Events)
    ├── Domain: rsvp.solstice.events
    └── events@rsvp.solstice.events
```
...
├── Pod  "inbox-triage"
│   ├── triage@openmail.sh
│   └── escalations@openmail.sh
└── Pod  "outreach-sequence"
    ├── sequence@openmail.sh
    └── followups@openmail.sh
```

<Note>
  Examples mix `@openmail.sh` and [custom domains](/concepts/custom-domains) on purpose — many teams start on OpenMail addresses and add a scoped domain per tenant when they go live.
</Note>
...
This means pods are fully backward compatible: if you never touch the pods API, everything keeps working and your inboxes live in the default pod.
...
curl https://api.openmail.sh/v1/pods/org_k9v2 \
...
curl "https://api.openmail.sh/v1/inboxes?podId=org_k9v2" \
  -H "Authorization: Bearer $OPENMAIL_API_KEY"
...
curl -X DELETE https://api.openmail.sh/v1/pods/org_k9v2 \
  -H "Authorization: Bearer $OPENMAIL_API_KEY"
```
...
<AccordionGroup>
  ...
  <Accordion title="Can each pod have its own custom domain?">
    Yes. Scope a domain to one pod and create inboxes on it only inside that pod — for example `alerts@mail.lumenlabs.co` in the Lumen Labs pod and `intake@inbox.waypoint.health` in the Waypoint Health pod. You can also add account-wide domains that every pod shares.
  </Accordion>
</AccordionGroup>
