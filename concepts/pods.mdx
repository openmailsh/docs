...
- **The account-wide key reaches everything.** The key you get at signup can read and write resources in *every* pod, so never hand it to an end user. Mint a [pod-scoped key](#pod-scoped-api-keys) instead — it can only reach its own pod.
...
- Revoking it is immediate — the key returns `401` on its next request.
...
Each pod has a cap on active keys. Hitting it returns `api_key_limit_reached` (422) — revoke an unused key first. You can also mint and revoke keys from a pod's page in the console (account owners only).
...
<AccordionGroup>
  ...
  <Accordion title="Can I restrict an API key to a single pod?">
    Yes. Mint a **pod-scoped key** with `POST /v1/pods/{id}/api-keys` or from the pod's page in the console. It can only read and send from that pod's inboxes; everything else returns `403`. The token is shown once at creation, and you can revoke the key at any time. See [Pod-scoped API keys](#pod-scoped-api-keys).
  </Accordion>
