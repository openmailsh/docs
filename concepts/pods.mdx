...
[Custom domains](/concepts/custom-domains) can be **account-wide** (usable by every pod) or **scoped to one pod**. When creating an inbox on a pod-scoped domain, the inbox must be in that same pod; account-wide domains work in any pod. Manage scope from the dashboard — see [Pod scoping](/concepts/custom-domains#pod-scoping).
...
Deleting a pod does not delete inboxes; it only removes an empty container. To offboard a customer, delete their inboxes, then the pod (you can address the pod by your own `clientId`):
...
# 3. Now the pod is empty and can be deleted
