...
**MAIL FROM records** (`bounce` MX + TXT) set the envelope sender on your domain so SPF can align. Publish both on the `bounce` host exactly as shown — do not put the SPF include on your apex unless the dashboard tells you to.
...
Once you've set your DNS records, click **Verify** in the dashboard. OpenMail checks your inbound MX, MAIL FROM (SPF), and DKIM records and updates the domain status to `verified`.
