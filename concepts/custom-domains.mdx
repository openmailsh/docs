...
| CNAME | `<token>._domainkey` (+ your subdomain) | Provided by OpenMail (three records) |
...
**DKIM records** are three CNAMEs under `_domainkey`. They add a cryptographic signature to outbound email, proving messages were sent by you. The hosts and targets are unique per domain and provided after you add it.
