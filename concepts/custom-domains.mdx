...
**DMARC record** tells receivers what to do with mail that fails SPF or DKIM. Google and Yahoo require bulk senders to publish one. We suggest `p=none;` — it satisfies the requirement and reports only, without affecting delivery. Tighten to `quarantine` or `reject` once you are confident in your alignment. If your domain already has a `_dmarc` record, keep it rather than replacing it. See [SPF, DKIM, DMARC](/pages/resources/security/email-protocols).
...
<Note>
  DMARC is your own published policy, so it is not part of verification — a domain verifies without it. We still show the record because bulk-sender requirements expect one.
</Note>
