...
Every inbox lands on your account's default domain unless you pass a verified [custom domain](/concepts/custom-domains):

```
{mailboxName}@omail.sh
```

New accounts get `@omail.sh`. Accounts created earlier keep `@openmail.sh` — that default is pinned at signup and does not change.
