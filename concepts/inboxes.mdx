...
- It **cannot read or change [sender rules](/concepts/sender-rules)**. Every `/v1/policy` call from a scoped key is a `403`, so who the inbox may correspond with is set by you and is neither visible nor editable to the agent.
