...
- It **cannot read or change [sender rules](/concepts/sender-rules)**. Every `/v1/policy` call from a scoped key returns `403`, so who the inbox may correspond with is set by you, not the agent.
