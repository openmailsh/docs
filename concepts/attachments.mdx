---
title: "Attachments"
description: "Send files with outbound emails and access attachments on inbound messages. OpenMail parses and stores attachments, making them available via API."
icon: paperclip
---
...
When an inbound email includes attachments, OpenMail stores them, extracts readable text, and provides download URLs you fetch with your API key.
...
This endpoint returns the attachment bytes directly. Authenticate with the same `Authorization: Bearer <API_KEY>` header you use for the rest of the API — it is not a redirect, so no separate signed URL or expiry to worry about.

```bash
curl https://api.openmail.sh/v1/attachments/msg_4c8d5e6f/receipt.pdf \
