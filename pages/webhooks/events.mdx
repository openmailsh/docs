...
OpenMail sends webhooks when inbound email is received, and when the reputation engine suspends or reactivates one of your inboxes or pods. Emails dropped by [sender rules](/concepts/sender-rules) (a block rule, or allowlist mode with no match) do not trigger events; they are discarded before threading and show up in the Allow/Block List [history](/concepts/sender-rules#history) instead.
...
```json
{
  ...
  "message": {
    ...
    "raw_url": "https://api.openmail.sh/v1/messages/msg_.../raw",
...
| `message.raw_url` | Download link for the original `message/rfc822` bytes, for verifying DKIM yourself. `null` when the original was not retained. Fetch it with your API key, same as attachment URLs. |
