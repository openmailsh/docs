...
`inbox.suspended`, `inbox.reactivated`, `pod.suspended`, and `pod.reactivated` share the structure below. They carry a `reputation` object instead of a `message`. For `pod.*` events, `inbox_id` and `inbox_address` are `null`.
...
| `reputation.scope` | `inbox` or `pod` — what was suspended/reactivated |
| `reputation.state` | The new reputation state (e.g. `suspended`, `watch`) |
| `reputation.reason` | Why it was suspended (e.g. `bounce_rate_high`); `null` on reactivation |
| `reputation.actor` | What triggered it: `engine`, `phishing`, or `admin` |
