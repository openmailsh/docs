...
Yes, in both directions. Sender rules block specific addresses or entire domains, and allowlist mode limits an inbox to approved correspondents. Inbound and outbound are set independently, at the account, pod, or inbox scope. Manage rules from **Allow/Block List** in the [Dashboard](https://console.openmail.sh/login) — changes are owner-only, and every change and refusal is recorded in the **History** audit trail. See [Sender rules](/concepts/sender-rules).
