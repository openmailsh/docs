...
For OpenClaw, run `openclaw plugins install clawhub:@openmail/openclaw` then `openclaw channels add --channel openmail --api-key <key>`. For other agents, install the CLI with `npm install -g @openmail/cli` and run `openmail init` to create your first inbox. See the [Quickstart](/quickstart) for the full walkthrough.
