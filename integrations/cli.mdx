...
One `--to`; repeat `--cc`. HTML in `--body` is detected. With `--thread-id` the subject is optional and the previous message is quoted unless you pass `--no-quote`. `--reply-to` on the free plan has to be an inbox you own; `--idempotency-key` makes retries safe.
...
- **`send` takes one `--to`.** More recipients go in `--cc`.
