...
`0` on success, `1` on anything else: bad flags, API errors, a deprecated command. With `--json` the error is a JSON object on stderr, so scripts can branch on the code and parse the reason.
