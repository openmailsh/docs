...
{
  "id": "inb_...",
  "address": "support@example.omail.sh",
  "displayName": "Support",
  "createdAt": "2024-01-01T00:00:00.000Z"
}
```
...
```bash
# Extract text (PDF, DOCX, XLSX, PPTX, images via OCR)
openmail attachments text --message-id "msg_..." --filename "report.pdf"

# Download the raw file
openmail attachments get --message-id "msg_..." --filename "report.pdf" --out ./report.pdf
```

Inside OpenClaw, prefix every command with `openclaw openmail --` to run it with the channel's credentials.
...
- **Pro plan or higher**: `replyTo` can be any valid email address,
