---
title: "references/errors.md"
description: "API error codes, CLI troubleshooting, suppression handling, and rate limit guidance for AI agents. Mirrored from the openmail skill package."
icon: alert-circle
---

Source: [skills/openmail/references/errors.md](https://github.com/openmailsh/skills/blob/main/skills/openmail/references/errors.md)
...
| 400 | `bad_request` (invalid `replyTo`) | The `replyTo` field is not a valid email address | Pass a syntactically valid email or omit the field |
| 401 | `unauthorized` | Invalid or missing API key | Check `OPENMAIL_API_KEY` is set and starts with `om_` |
| 403 | `feature_requires_paid_plan` (`external_reply_to`) | Free plan tried to set `replyTo` to an address that isn't one of their own OpenMail inboxes | Create the destination inbox and use its address, or upgrade to Pro or higher to use any external address |
| 403 | `forbidden` | The API key is scoped to one pod or one inbox, and this request touched something outside it — or an action its scope forbids, like creating or deleting an inbox | The `message` names the boundary that was crossed. Use the key belonging to that pod or inbox, or an account-wide key if the action needs one |
...
| 422 | `api_key_limit_reached` | The pod or inbox already has 20 active API keys | Revoke an unused key to free a slot |
| 429 | `rate_limit_exceeded` | Too many requests | Check `Retry-After` header and wait before retrying |
| 429 | `cold_outreach_limit` | Cold send limit exceeded for new inbox | New inboxes have a warm-up period; see [email deliverability docs](/best-practices/email-deliverability) |

## CLI errors

### `missing inbox id; run 'openmail init' or pass --inbox-id`

No default inbox is saved. Run:

```bash
openmail init --mailbox-name "my-agent"
```

Or pass `--inbox-id` on the command. Inside OpenClaw the plugin supplies the inbox; use `openclaw openmail -- <command>`.
...
```bash
echo $OPENMAIL_API_KEY
```

The key should start with `om_`. Export it again or pass `--api-key` if it is wrong or missing. Inside OpenClaw, re-run `openclaw channels add --channel openmail --api-key <key>`.
...
To view or manage suppressions, go to **Settings > Suppressions** in the [dashboard](https://console.openmail.sh/login).
